...
|---|---|---|---|---|
| 正式 | `-Pchannel=release`（默认） | `3.12` | `31200` | GitHub Release 分发 |
| 测试 | `-Pchannel=local` | `3.12.99-local` | `31299` | 本机构建、真机自测 |

规则与理由：

- **测试版号必须恒大于同一 patch 的正式版**，否则装过测试版就回不去正式版——
  测试版取 `.99-local`（`31299 > 31200`），保证可原地覆盖安装正式版做对比验证；
- **同时必须恒小于下一个次版本正式版**（3.13 → `31300`），否则测试版会把后续
