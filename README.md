...
- **后台静默调用**（v3.13）：API 调用默认**不再把小布界面拉到前台**——小布在后台
  ...
  仅当后台整轮确实收不到回调（典型是**小布进程刚起、对话引擎尚未预热**）时，
  才降级拉一次前台并重试；锁屏下唤醒失败会如实返回 `503 not_foreground`
- **自动唤醒（降级兜底）**：即上条的兜底开关。关闭后后台失败不再拉前台，
...
|---|---|---|---|---|
| 正式 | `-Pchannel=release`（默认） | `3.13` | `31300` | GitHub Release 分发 |
| 测试 | `-Pchannel=local` | `3.13.99-local` | `31399` | 本机构建、真机自测 |

规则与理由：

- **测试版号必须恒大于同一 patch 的正式版**，否则装过测试版就回不去正式版——
  测试版取 `.99-local`（`31399 > 31300`），保证可原地覆盖安装正式版做对比验证；
- **同时必须恒小于下一个次版本正式版**（3.14 → `31400`），否则测试版会把后续
...
5. `GatewayWatchdog` 常驻线程每秒比对配置与监听状态，负责网关实时启停与心跳探活
6. 结果按 OpenAI 兼容格式（可选 SSE chunked 流式）返回给客户端
